...
class AIModelInterface:
    ...
    def predict(self, frame):
        ...
        return [{"bbox": box.xyxy[0].tolist(), "confidence": box.conf[0].item(), "class_id": int(box.cls[0].item()), "label": self.model.names[int(box.cls[0].item())]} 
                for result in results if result.boxes is not None for box in result.boxes]